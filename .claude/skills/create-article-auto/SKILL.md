...
**NE JAMAIS ecrire le nom du site dans le `title` du frontmatter.** Le theme l'ajoute deja tout seul : `themes/comparatif-pro/layouts/_default/baseof.html` rend `{{ .Title }} | {{ .Site.Title }}`. Mettre le suffixe dans le frontmatter le fait sortir **deux fois** dans la balise, et comme le H1 est construit sur `.Title` a defaut de `.Params.h1`, le nom du site se retrouve aussi **dans le H1**. Defaut constate et corrige sur `meilleur-logiciel-sirh.com` le 2026-08-11 (16 articles) et sur `magazine-como` le 2026-09-01 (4 pages).

Ce theme passe le titre dans un partial avant de le rendre : `{{ partial "dyn.html" .Title }} | {{ .Site.Title }}`. Le partial peut substituer l'annee courante dans le texte, donc **lire `layouts/partials/dyn.html` avant de calculer la longueur** si le title contient un millesime.
...
|---|---|---|---|
| fr | Comparatif-Pro | ` \| Comparatif-Pro` (17) | **43** |
| en | Comparatif-Pro | ` \| Comparatif-Pro` (17) | **43** |
