...
> **In short:**
> 1. The approved platform transmits, it does not advise and never rules on your compliance.
> 2. The integrator connects your existing system, and is only needed if that system is non-standard.
> 3. The firm establishes the scope, arbitrates and gives an opinion on tax compliance. It is the only one of the three that does.
> 4. The simplest setup is a firm that already embeds a registered platform, so you keep a single provider accountable for the outcome.
...
Outside these cases, ask the question plainly, and be wary of an answer that always concludes integration is required. Our [guide to approved platforms and their registration](/en/blog/pdp-electronic-invoice/) sets out what the standard connection already covers.
...
- **The firm** should supply a written scope assessment stating which invoices are affected and by when. Several networks offer a [readiness self-assessment](https://www.inextenso.fr/diagnostic-facture-electronique/) beforehand, so you arrive at the conversation with an initial picture.
...
1. **Map your existing invoicing process**, both issuing and receiving.
2. **Establish your scope of obligation** and the dates that actually apply to you.
3. **Choose the platform** in light of the two previous points, not the other way round.
4. **Connect**, with an integrator only if your system warrants one.
5. **Train**, and plan a period of parallel running before switching over.
